 ...
 *
 * Included by cib-civievent-shortcode.php and cib-civievent-single-widget.php.
 */
...
function cib_fetch_event_location($event_id)
{
  try {
    ...
    $addr = civicrm_api3('Address', 'getsingle', [
      'id' => $address_id,
      'return' => ['street_address', 'supplemental_address_1', 'city', 'postal_code', 'state_province_id.abbreviation', 'country_id.name'],
    ]);
 ...
 */
function cib_apply_event_template($template, $event, $index = 0, $image_field = '', $date_format = '', $time_format = '', $location = [], $event_page = '/events/event-single/')
{
  $event_id = intval($event['id']);
  $url = rtrim($event_page, '/') . '/?eventID=' . $event_id;
  $reg_url = CRM_Utils_System::url('civicrm/event/register', "reset=1&id=$event_id");
  ...
  // ── Image ────────────────────────────────────────────────────────────────────
  $image_url = $image_field && !empty($event[$image_field]) ? $event[$image_field] : '';
  $image_tag = $image_url
    ? '<img src="' . esc_url($image_url) . '" alt="' . esc_attr($event['title'] ?? '') . '" style="width:100%;height:100%;object-fit:cover;display:block;" />'
    : '';

  // ── Shared button styles (Astra-compatible) ──────────────────────────────────
  $btn = 'display:inline-block;padding:7px 16px;border-radius:6px;text-decoration:none;font-size:.83em;font-weight:600;margin:4px 4px 4px 0;';
  $btn_primary = $btn . 'background:var(--ast-global-color-0,#4169e1);color:#fff;';
  $btn_outline = $btn . 'border:1.5px solid var(--ast-global-color-0,#4169e1);color:var(--ast-global-color-0,#4169e1);';

  // ── Calendar ─────────────────────────────────────────────────────────────────
  $ical_url = $gcal_url = $calendar_links = '';
  if ( $event['is_show_calendar_links'] ?? false){
    $ical_url = CRM_Utils_System::url('civicrm/event/ical', "reset=1&id=$event_id");
    $gcal_params = [
      'action' => 'TEMPLATE',
      'text' => $event['title'] ?? '',
    ...
    $gcal_url = 'https://www.google.com/calendar/render?' . http_build_query($gcal_params);
  ...
  $share_twitter =
    '<a href="https://x.com/intent/tweet?url=' . $enc_url . '&amp;text=' . $enc_title . '" target="_blank" rel="noopener" style="' . $btn_primary . '">X / Twitter</a>';
  $share_facebook = '<a href="https://www.facebook.com/sharer/sharer.php?u=' . $enc_url . '" target="_blank" rel="noopener" style="' . $btn_primary . '">Facebook</a>';
  $share_linkedin =
    ...
    $btn_primary .
    '">LinkedIn</a>';
  $share_email = '<a href="mailto:?subject=' . rawurlencode($event['title'] ?? '') . '&amp;body=' . $enc_text . '%0A' . $enc_url . '" style="' . $btn_primary . '">Email</a>';
  $social_links = ($event['is_share'] ?? false) ? '<div style="display:flex;flex-wrap:wrap;gap:6px;">' . $share_twitter . $share_facebook . $share_linkedin . $share_email . '</div>' : '';
  ...
  if ( $event['is_map'] ?? false){
    if (!empty($location['full'])) {
      $map_url = 'https://www.google.com/maps/search/?api=1&query=' . rawurlencode($location['full']);
      $map_link = '<a href="' . esc_url($map_url) . '" target="_blank" rel="noopener" style="' . $btn_outline . '">&#x1F4CD; Map</a>';
    }
  }
  ...
  if (
    ...
    (empty($event['registration_start_date']) || strtotime($event['registration_start_date']) <= time()) &&
    (empty($event['registration_end_date']) || strtotime($event['registration_end_date']) > time())
  ) {
    $link_text = esc_html($event['registration_link_text'] ?? 'Register');
    $register_buttons .= " <a href='" . esc_url($reg_url) . "' title='Register Now' class='button btn'><span>" . $link_text . '</span></a>';
  }
  ...
  $event_data = [
    ...
    'url'                    => esc_url($url),
    'register_url'           => esc_url($reg_url),
    'registration_link_text' => esc_html($event['registration_link_text'] ?? 'Register'),
    ...
    'location_address'       => ($event['is_show_location'] ?? false) ? esc_html($location['address'] ?? '') : '',
    'location_city'          => ($event['is_show_location'] ?? false) ? esc_html($location['city'] ?? '') : '',
    'location_state'         => ($event['is_show_location'] ?? false) ? esc_html($location['state'] ?? '') : '',
    'location_country'       => ($event['is_show_location'] ?? false) ? esc_html($location['country'] ?? '') : '',
    'location_postal'        => ($event['is_show_location'] ?? false) ? esc_html($location['postal'] ?? '') : '',
    'location_full'          => ($event['is_show_location'] ?? false) ? esc_html($location['full'] ?? '') : '',
