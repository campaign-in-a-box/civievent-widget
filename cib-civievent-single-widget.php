...
class civievent_single_Widget extends civievent_Widget
{
  ...
  public function __construct($shortcode = false)
  {
    WP_Widget::__construct(
      ...
      // Args.
      ['description' => __('displays a single CiviCRM event', 'civievent-widget')],
    );
  ...
  public function create_widget($args, $instance)
  {
    ...
    if (isset($event['is_online_registration']) && $event['is_online_registration'] == 1) {
      ...
      $content .=
        ...
        'civicrm/event/register/?id=' .
        $event['id'] .
        "&amp;reset=1' title='Register Now' class='button btn'>Register Now</a>";
    ...
    $content .=
      "  <p>Please help us and let your friends, colleagues and followers know about <strong><a href='/events/event-single/?eventID=" .
      $event['id'] .
      "'>" .
      $event['title'] .
      '</a></strong></p>';
    $content .= "  <div class='civievent-widget-button-section'>";
    $content .=
      '   <button onclick="window.open(\'https://X.com/intent/tweet?url=' .
      $infoLink .
      '&amp;text=' .
      $TXT .
      '\',\'_blank\')" type="button" class="button btn btn-default" id="crm-tw" title="Share">Twitter</button>';
    $content .=
      '   <button onclick="window.open(\'https://facebook.com/sharer/sharer.php?u=' .
      $infoLink .
      '\',\'_blank\')" type="button" class="button btn btn-default" role="button" id="crm-fb" title="Share">Facebook</button>';
    $content .=
      '   <button onclick="window.open(\'https://www.linkedin.com/shareArticle?mini=true&amp;url=' .
      $infoLink .
      '&title=' .
      $TXT .
      '\',\'_blank\')" type="button" rel="noopener" class="button btn btn-default" id="crm-li" title="Share">LinkedIn</button>';
    $content .=
      '   <button onclick="window.open(\'mailto:?subject=' .
      $event['title'] .
      '&amp;body=' .
      $TXT .
      '%0A' .
      $infoLink .
      '\',\'_self\')" type="button" rel="noopener" class="button btn btn-default" id="crm-email" title="Share">Email</button>';
  ...
  public function form($instance)
  {
    if (!function_exists('civicrm_initialize')) { ?>
      ...
      <div class="civievent-widget-nobasepage">
  	  	...
  	  	<?php
      print '<p>' .
        __('You do not have a WordPress Base Page set in your CiviCRM settings.  This can cause the CiviEvent Widget to display inconsistent links.', 'civievent-widget');
    ...
    ?>
      ...
      'wtheme',
  		...
  		<?php _e(
      'Enter the theme for the widget.	The standard option is "standard", or you can enter your own value, which will be added to the widget class name.',
      'civievent-widget',
    ); ?>
		</p>
		<p>
  		...
  		<input class="widefat" id="<?php echo $this->get_field_id(
      'offset',
    ); ?>" name="<?php echo $this->get_field_name('offset'); ?>" type="text" value="<?php echo esc_attr($offset); ?>" />
  	  <?php _e(
       'By default, the widget will show the first upcoming event starting today or in the future.  Enter an offset to skip one or more events: for example, an offset of 1 will skip the first event and display the second.',
       'civievent-widget',
     ); ?>
		</p>
		<p>
