(function ($) {
  $(document).on("ready widget-updated widget-added", function () {
    ...
    $(
      'input[name^="widget-civievent-widget["][name$="][admin_type]"]:checked',
    ).each(function () {
      adminTypeSwitch($(this));
    });
    $('input[name^="widget-civievent-widget["][name$="][admin_type]"]').change(
      function () {
        adminTypeSwitch($(this));
      },
    );
    function adminTypeSwitch($field) {
      $field
        .siblings('label[for!="' + $field.attr("id") + '"]')
      ...
      if ($field.val() == "custom") {
        $field
          .siblings(".civievent-widget-admin-custom")
          .addClass("selectedTab");
        $field
          .siblings(".civievent-widget-admin-simple")
          .removeClass("selectedTab");
      } else {
        $field
          .siblings(".civievent-widget-admin-simple")
      ...
      for (var field in existing) {
        if (existing.hasOwnProperty(field)) {
          ...
          var $fieldtitle = $("<div/>", {
            ...
            $(".field-custom-ui.active-ui-field").removeClass(
              "active-ui-field",
            );
            if (!unsetClass) {
          ...
          if (
            existing[field].hasOwnProperty("title") &&
            parseInt(existing[field]["title"])
          ) {
          ...
          if (
            existing[field].hasOwnProperty("wrapper") &&
            parseInt(existing[field]["wrapper"])
          ) {
